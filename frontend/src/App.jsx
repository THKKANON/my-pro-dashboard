...
function App() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!allChartData) setIsLoading(true);
            try {
                // ✨✨✨ 핵심 수정: API 주소 오타 수정 ✨✨✨
            ...
            } catch (error) { console.error("Failed to fetch chart data:", error); setAllChartData({});
    ...
    const getSlicedDataForChart = (tf) => {
        if (!allChartData || !allChartData[tf]) return null;
        ...
        const generateMarkPoints = (signalTimestamps, signalType, color) => {
            ...
            let symbolPath = 'pin';
            let yAxisCoord, symbolOffset;

            if (signalType.includes('buy')) {
                symbolPath = upArrowPath; yAxisCoord = 'low'; symbolOffset = [0, 10];
            } else if (signalType.includes('sell')) {
                symbolPath = downArrowPath; yAxisCoord = 'high'; symbolOffset = [0, -10];
            } else if (signalType.includes('long')) {
                symbolPath = starPath; yAxisCoord = 'low'; symbolOffset = [0, 12];
            } else { // short
                symbolPath = starPath; yAxisCoord = 'high'; symbolOffset = [0, -12];
            }
            
            return signalTimestamps
                .map(ts => {
                    if (candleTimeMap.has(ts)) {
                        ...
                        return {
                            xAxis: candleIndex,
                            yAxis: candle[yAxisCoord === 'low' ? 'Low' : 'High'],
                            symbol: symbolPath,
        ...
        <div className="app-container">
            ...
            {isLoading ? (<div className="loading-text">Loading initial data...</div>) : (
                <div className="chart-grid">
                    {selectedTfs.map((tf, i) => (
                        <div className="chart-wrapper" key={`${selectedCoin}-${tf}-${i}`}>
                            <ChartComponent data={getSlicedDataForChart(tf)} title={`TF${i + 1}: ${tf}`} />
                        </div>
                    ))}
                </div>
            )}
